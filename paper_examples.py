...
# \\\\\\\\\\\\\\\\\\\\ Generate Lorenz 63 example in Figure 1 \\\\\\\\\\\\\\\\\\\\\\\\
...
fig, ax = create_sigma_histogram(
    ...
    plot_params,
    save_plot=True,
    file_name="figures/sigma_b_bound_distribution.png",
)


# \\\\\\\\\\\\\\\\\\\\\\ Generate Lorenz 63 example in Figure 2 \\\\\\\\\\\\\\\\\\\\\\\\
...
pair_levelsmisfit_inflate = pd.concat(
    map(lambda df: df.misfit, noise_inflate_dfs), axis=1
)
pair_rel_error_inflate = pd.concat(
    map(lambda df: df.relative_error, noise_inflate_dfs), axis=1
)
...
fig, ax, pivot = create_rmse_heatmap(
    ...
    plot_params,
    save_plot=True,
    file_name="figures/obs_inflation_levels_l63.png",
)


# \\\\\\\\\\\\\\\\\\\\\\\ Generate Lorenz 63 example in Figure 3 \\\\\\\\\\\\\\\\\\\\\\\\
...
config_avg = Lorenz63Config.default_config(
    total_steps=1000, window_size=20, obs_frequency=4, obs_std=2.0, inflation_factor=12
)
...
# \\\\\\\\\\\\\\\\\\\\\\\ Generate Lorenz 63 example in Figure 4 \\\\\\\\\\\\\\\\\\\\\\\\
...
for obs_freq in obs_freqs:
    ...
    for metric, storage in zip(
        metrics, [rmse_noise_dfs, misfit_noise_dfs, rel_error_noise_dfs]
    ):
...
obs_df = pd.concat(
    [df.assign(Observation_Density=freq) for df, freq in zip(rmse_noise_dfs, obs_freqs)]
)
...
plot_cross_val(
    dci_wme_obs.T, plot_params, save_plot=True, file_name="dci_wme_noisevdensity.png"
)


# \\\\\\\\\\\\\\\\\\\\\\\ Generate Lorenz 96 example in Figure 5 \\\\\\\\\\\\\\\\\\\\\\\\
...
# \\\\\\\\\\\\\\\\\\\\\\\ Generate Lorenz 96 example in Figure 6 \\\\\\\\\\\\\\\\\\\\\\\\
...
rmse_scores_analysis = {
    m: rmse(results_avg_96["analysis"][m], results_avg_96["trajectory"]) for m in models
}
bias_scores_analysis = {
    m: mbe(results_avg_96["analysis"][m], results_avg_96["trajectory"]) for m in models
}
...
# \\\\\\\\\\\\\\\\\\\\\\\ Generate Lorenz 96 example in Figure 7 \\\\\\\\\\\\\\\\\\\\\\\\
cost_functions = {
    "Bayes": bayes_cost_function,
    "DCI": dci_cost_function,
    "DCI_WME": dci_wme_cost_function,
...
# \\\\\\\\\\\\\\\\\\\\\\\ Generate Lorenz 96 example in Figure 8 \\\\\\\\\\\\\\\\\\\\\\\\
